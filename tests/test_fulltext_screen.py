...
def test_screen_xml_reports_hash_and_protocol_signal() -> None:
    result = screen_xml("PMC1", STRONG_XML, "development")
    assert result["machine_screen_status"] == "strong_protocol_signal"
...
def test_screen_plan_keeps_scope_explicit() -> None:
    ...
    assert result["status_counts"] == {
        "strong_protocol_signal": 1,
        "weak_or_no_protocol_signal": 1,
    }
    assert all(
